...
def _get_instance_names(ch):
    ...
    try:
        ta_list = ch.EnumerateInstanceNames('TestAtom')
    except pywbem.CIMError, arg:
        raise
    return ta_list
...
################################################################################
        ...
        ta_list = []
        try: 
            ta_list = ch.EnumerateInstances('TestAtom')
        except pywbem.CIMError, arg:
            raise 'EnumerateInstances failed: %s' % str(arg)
        try:
            paths = ch.EnumerateInstanceNames('TestAtom')
        except pywbem.CIMError, arg:
...
################################################################################
        ...
        try: 
            instances = ch.EnumerateInstances('TestAtom')
        except pywbem.CIMError, arg:
            raise 'EnumerateInstances Failed: %s' % str(arg)

	...
	switch (pegval.getType())
	{
		...
		case CIMTYPE_BOOLEAN:
		{
			...
			else
			{
				Boolean b;
				pegval.get(b);
				ro = (b) ?  Py::Object(Py_True) : Py::Object(Py_False);
			}
			break;
		}
		case CIMTYPE_REAL32:
...
PGPyConv::PGQual2Py(const CIMConstQualifier& qual)
{
	...
	pyarg[3] = _bool2Py(bool(qual.getPropagated()));				// propagated
	const CIMFlavor& flavor = qual.getFlavor();
	pyarg[4] = _bool2Py(flavor.hasFlavor(CIMFlavor::OVERRIDABLE));
	pyarg[5] = _bool2Py(flavor.hasFlavor(CIMFlavor::TOSUBCLASS));
	pyarg[6] = _bool2Py(flavor.hasFlavor(CIMFlavor::TOINSTANCE));
	pyarg[7] = _bool2Py(flavor.hasFlavor(CIMFlavor::TRANSLATABLE));
	return pyfunc.apply(pyarg);
}
...
PGPyConv::PGQualType2Py(const CIMConstQualifierDecl& qualt)
{
	...
	pyarg[6] = _bool2Py(cflavor.hasFlavor(CIMFlavor::OVERRIDABLE));
	pyarg[7] = _bool2Py(cflavor.hasFlavor(CIMFlavor::TOSUBCLASS));
	pyarg[8] = _bool2Py(cflavor.hasFlavor(CIMFlavor::TOINSTANCE));
	pyarg[9] = _bool2Py(cflavor.hasFlavor(CIMFlavor::TRANSLATABLE));
	return pyfunc.apply(pyarg);
}
...
PGPyConv::PGCIMParam2Py(const CIMConstParameter& param)
{
	...
	pyarg[3] = _bool2Py(param.isArray());				// is_array
...
PGPyConv::PGMeth2Py(const CIMConstMethod& meth)
{
	...
	pyarg[4] = _bool2Py(meth.getPropagated());
...
PGPyConv::PGProperty2Py(const CIMConstProperty& prop)
{
	...
	pyarg[5] = _bool2Py(prop.getPropagated());	// propagated
	pyarg[6] = _bool2Py(prop.isArray());		// is_array

...
def get_instance_names(conn):
    ...
    try:
        ta_list = conn.EnumerateInstanceNames('TestAtom')
    except pywbem.CIMError, arg:
        raise 
        return None
...
def get_test_instances(conn):
    ...
    try:
        ta_list = conn.EnumerateInstances('TestAtom')
    except pywbem.CIMError, arg:
        raise 
        return None
...
class TestAtomProvider(unittest.TestCase):
    ...
    def test_3_enum_instances(self):
        ...
        try: 
            ta_list = self.conn.EnumerateInstances('TestAtom')
        except pywbem.CIMError, arg:
            self.fail('EnumerateInstances Failed: %s' % str(arg))
            raise

        try:
            paths = self.conn.EnumerateInstanceNames('TestAtom')
        except pywbem.CIMError, arg:
    ...
    def test_4_enum_instance_names(self):
        ...
        try: 
            ta_list = self.conn.EnumerateInstanceNames('TestAtom')
        except pywbem.CIMError, arg:
            self.fail('EnumerateInstanceNames Failed: %s' % str(arg))
            raise

        try: 
            instances = self.conn.EnumerateInstances('TestAtom')
        except pywbem.CIMError, arg:
            self.fail('EnumerateInstances Failed: %s' % str(arg))
